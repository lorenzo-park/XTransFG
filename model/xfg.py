...
from model.vit import FFN, Transformer, np2th, Block, VisionTransformer
...
class XFGConcatWithBackbone(nn.Module):
    def __init__(self, config, num_classes=200, zero_head=False):
        super(XFGConcatWithBackbone, self).__init__()
    ...
    def forward(self, img_tokens, txt_tokens):
        cls_token = self.cls_token.expand(img_tokens.shape[0], -1, -1)
...
class XFGCrossAttnWithBackbone(nn.Module):
    def __init__(self, config, num_classes=200, zero_head=False):
        super(XFGCrossAttnWithBackbone, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
