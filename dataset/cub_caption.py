...
class CapCUB200(datasets.VisionDataset):
  def __init__(self, root: str, tokenizer_path: str, train: bool = True, target_transform: Optional[Callable] = None):
  ...
  def __getitem__(self, index: int) -> Any:
    input_id, mask, target = self.data[index]
  ...
  def parse_data_file(self, root: str, train: bool) -> List[Tuple[str, int]]:
    ...
    data_list = []
    for input_id, mask, split, label in zip(tokens.input_ids, tokens.attention_mask, splits, labels):
      ...
      if int(split) == int(train):
        ...
        data_list.append((input_id, mask, int(label)-1))
